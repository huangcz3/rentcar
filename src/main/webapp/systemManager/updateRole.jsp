...
<body>
	...
	<form action="updateRoleMenus" method="post" class="form form-horizontal"
		id="form-admin-role-add">
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span
				class="c-red">*</span>角色名称：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="${role.roleid} " placeholder=""
					id="roleid" name="roleid"  readonly="readonly">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-3"><span
						...
						<dl class="cl permission-list2">
							<dt>
								...
								<%-- <c:forEach items="${rmlist }" var="rm"> --%>
									...
									<input type="checkbox" value="${menus.menuid }"
									name="menuid" id="user-Character-0-0">${menus.menuname } 
								</label>
								<%-- </c:forEach> --%>
							</dt>
							<c:forEach items="${menus.childMenu }" var="menu2">
							<dd>
								<label class=""> <input type="checkbox" value="${menu2.menuid }"
									name="menuid" id="user-Character-0-0-0">${menu2.menuname }
	...
	<script type="text/javascript"
		...
		<script type="text/javascript">
			$(function() {
				...
				var array = new Array();  
				<c:forEach items="${rmlist}" var="t">  
					array.push("${t.menuid}"); //js中可以使用此标签，将EL表达式中的值push到数组中  
				</c:forEach> 
					
				for (var i = 0; i < array.length; i++) {
					var me= array[i];
					$("input[type='checkbox'][value='"+me+"']").attr("checked","true"); 
				}
